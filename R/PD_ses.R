...
PD_ses <- function(x, phy, iter = 1000, ...){
    ...
    PD_obs <- as.vector(data.frame(PD=PD(x, phy))$PD)
    pd_rand <- t(replicate(iter, as.vector(data.frame(PD=PD(x, rand_tip(phy)))$PD)))
    ...
    y <- data.frame(table(sparse2long(x)$grids))
    names(y) <- c("grids", "ntaxa")
    return(data.frame(y, PD_obs,
                      pd_rand_mean, pd_rand_sd, pd_obs_rank, pd_obs_z,
