...
#' }
#' @export
sdm_to_delete <- function(x, pol = NULL, predictors = NULL, blank = NULL, tc = 2,
                lr = 0.001, bf = 0.75, n.trees = 50, step.size = n.trees, k=3,
    ...
    if (length(x1) > 0) {
        ...
        rf1 <- suppressWarnings(invisible(randomForest::randomForest(Formula,
            ...
            if (maxent_available) {
                ...
                models <- list(RF, GLM, MX)
                models <- do.call(stack, models)
                names(models) <- c("RF", "GLM", "MAXENT")
                ...
                aucs <- c(auc_RF=erf@auc, auc_GLM=ge@auc)
                m_auc <- median(aucs)
