...
Th advantages of the sparse representation is that the community matrix can be 
stored much more memory efficient and also computatuions with a sparse matrix 
are also considerably faster. The phyloregion and Martrix package contains some 
functions to conveniently change between formats. 
```{r, eval=TRUE}
library(Matrix) 
data(africa)
